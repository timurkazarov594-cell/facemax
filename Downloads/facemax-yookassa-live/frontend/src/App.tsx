...
import { startPayment, checkPaymentStatus, unlockAfterPaymentSuccess, DEMO_MODE } from '@/lib/payment';
...
function FaceMaxApp() {
  ...
  const handleUnlock = async () => {
    ...
    // Demo mode: unlock immediately, show notice
    if (result.demo) {
    ...
    if (result.error || !result.confirmationUrl || !result.paymentId) {
      console.warn('[FACEMAX] Payment creation failed:', result.error);
      return;
    }

    // Save paymentId + current analysis state so we can restore after redirect
    try {
    ...
    // After payment, YooKassa redirects back to return_url (this app).
    // On return, the useEffect below polls for payment.status === 'succeeded'.
            ...
            >
                ...
                >
                  ...
                  <AnimatePresence>
                    {showPaymentComingSoon && (
                      ...
                      >
                        <p className="text-sm text-purple-200/65 leading-relaxed">
                          Оплата скоро будет подключена.<br />Полный анализ пока недоступен.
                        </p>
                        <button
                          onClick={() => setShowPaymentComingSoon(false)}
                          className="text-xs text-purple-400/35 mt-2 hover:text-purple-300/55 transition-colors"
                        >
                          Понятно
                        </button>
                      </motion.div>
              ...
              {isPremiumUnlocked && (
                ...
                >
                  {/* Demo-mode notice */}
                  <AnimatePresence>
                    {showDemoNotice && DEMO_MODE && (
                      <motion.div
                        initial={{ opacity: 0, y: -8 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -8 }}
                        transition={{ duration: 0.35 }}
                        className="flex items-center justify-between gap-3 py-3 px-5 rounded-2xl mb-6"
                        style={{
                          background: 'rgba(124,58,237,0.08)',
                          border: '1px solid rgba(168,85,247,0.2)',
                        }}
                      >
                        <span className="text-sm text-purple-300/70">Демо-доступ для проверки сервиса.</span>
                        <button
                          onClick={() => setShowDemoNotice(false)}
                          className="text-purple-400/40 hover:text-purple-300/60 transition-colors flex-shrink-0"
                          aria-label="Закрыть"
                        >

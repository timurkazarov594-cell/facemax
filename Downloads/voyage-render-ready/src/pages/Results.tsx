...
const TRUSTED_DOMAINS = ['booking.com', 'tp.media', 'travelpayouts.com', 'hotellook.com', 'search.hotellook.com', 'agoda.com', 'expedia.com'];
...
export default function Results() {
  ...
  const [bookingModalUrl, setBookingModalUrl] = useState<string | null>(null);
  ...
  // Open external link via the confirmation modal
  const openBookingLink = (url: string) => {
    if (!isTrustedUrl(url)) {
      toast({ title: t('results.hotel.untrusted'), variant: 'destructive' });
      return;
    }
  ...
  const handleBookingConfirm = () => {
    const url = bookingModalUrl;
    setBookingModalUrl(null);
    if (!url) return;
  ...
  return (
    <Layout>
      ...
      <BookingRedirectModal
        ...
        onClose={() => setBookingModalUrl(null)}
      />
    </Layout>
  );
}
